...
describe("ゆいまテスト", () => {
  ...
  it("サイトルートの疎通テスト", async () => {
    ...
    await driver.get("https://yuima-okinawa.jp/");

    // トップページのロード待ち
    //await driver.wait(until.titleContains('セキュリテ - インパクト投資プラットフォーム'), 10000);
    
    await driver.getTitle().then(function (title) {
      expect(title).toBe("YUIMA　沖縄から世界を拓くクラウドファンディング");
    });
  ...
  it("メール疎通テスト", async () => {
    ...
    const successUrl = "https://yuima-okinawa.jp/member/recover/login/thanks";
