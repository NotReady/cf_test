...
describe("デモ", () => {
  ...
  it("サイトルートの疎通テスト", async () => {
    ...
    await driver.get("https://sandwichcrowd.com/");

    // トップページのロード待ち
    //await driver.wait(until.titleContains('セキュリテ - インパクト投資プラットフォーム'), 10000);
    
    await driver.getTitle().then(function (title) {
      expect(title).toBe("大分のクラウドファンディング｜sandwich（サンドイッチ）");
    });
  ...
  it("メール疎通テスト", async () => {
    ...
    const successUrl = "https://sandwichcrowd.com/member/recover/login/thanks";
