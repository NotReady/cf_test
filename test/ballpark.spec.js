...
describe("デモ", () => {
  ...
  it("サイトルートの疎通テスト", async () => {
    ...
    await driver.get("https://ballpark.city/");

    // トップページのロード待ち
    //await driver.wait(until.titleContains('セキュリテ - インパクト投資プラットフォーム'), 10000);
    
    await driver.getTitle().then(function (title) {
      expect(title).toBe("ボールパークシティ　北海道共同創造プラットフォーム");
    });
  ...
  it("ログインページの遷移テスト", async () => {
    ...
    // ログインのリンク要素
    var aButton = await driver.findElement(By.xpath("/html/body/header/div/div/div/span/a"));
  ...
  it("メール疎通テスト", async () => {
    ...
    const successUrl = "https://ballpark.city/member/recover/login/thanks";
